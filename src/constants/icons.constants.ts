export const AppIcons = {
  LANGUAGE: "translate",
  EDUCATION_LEVEL: "book-education",
  ARROW_UP: "arrow-up",
  FACE_REPOSITORY: "account-box-edit-outline",
  SUCCESS: "check",
  TEACHER: "account-tie",
  SCHOOL: "school",
  STUDENT: "account-school",
  STAFF: "account-group",
  SUPERVISOR: "account-tie-hat",
  SUPERUSER: "shield-account",
  SCHOOL_MANAGEMENT: "office-building",
  MOE: "DOMAIN",
  ERROR: "alert-circle-outline",
  WARNING: "alert-box-outline",
  INFO: "information-box-outline",
  CONFIRM: "help",
  DASHBOARD: "view-dashboard",
  MOON: "weather-night",
  SUN: "weather-sunny",
  STUDENTS: "account-group",
  TEACHERS: "account-tie",
  CLASSES: "school",
  SETTINGS: "cog",
  PROFILE: "account",
  ACCOUNT: "account-cog",
  CHEVRON_UP: "chevron-up",
  CHEVRON_DOWN: "chevron-down",
  RELOAD: "reload",
  DOTS_CIRCLE: "dots-circle",
  DOTS_VERTICAL: "dots-vertical",
  PLUS: "plus",
  EDIT: "pencil",
  DELETE: "delete",
  SAVE: "content-save",
  CANCEL: "close",
  SEARCH: "magnify",
  FILTER: "filter",
  SORT: "sort",
  EXPORT: "export",
  IMPORT: "import",
  PRINT: "printer",
  DOWNLOAD: "download",
  UPLOAD: "upload",
  EYE: "eye",
  EYE_OFF: "eye-off",
  LOCK: "lock",
  UNLOCK: "lock-open",
  BELL: "bell",
  BELL_OFF: "bell-off",
  CALENDAR: "calendar",
  CLOCK: "clock",
  USER: "account",
  USERS: "account-group",
  HOME: "home",
  LOGOUT: "logout",
  LOGIN: "login",
  REGISTER: "account-plus",
  FORGOT_PASSWORD: "key",
  RESET_PASSWORD: "key-change",
  EMAIL: "email",
  PHONE: "phone",
  LOCATION: "map-marker",
  LINK: "link",
  ATTACHMENT: "attachment",
  MENU: "menu",
  IMAGE: "image",
  FILE: "file",
  FOLDER: "folder",
  STAR: "star",
  STAR_OUTLINE: "star-outline",
  HEART: "heart",
  HEART_OUTLINE: "heart-outline",
  THUMBS_UP: "thumb-up",
  THUMBS_DOWN: "thumb-down",
  COMMENT: "comment",
  SHARE: "share",
  COPY: "content-copy",
  CUT: "content-cut",
  PASTE: "content-paste",
  UNDO: "undo",
  REDO: "redo",
  BOLD: "format-bold",
  ITALIC: "format-italic",
  UNDERLINE: "format-underline",
  STRIKETHROUGH: "format-strikethrough",
  ALIGN_LEFT: "format-align-left",
  ALIGN_CENTER: "format-align-center",
  ALIGN_RIGHT: "format-align-right",
  ALIGN_JUSTIFY: "format-align-justify",
  LIST_BULLETED: "format-list-bulleted",
  LIST_NUMBERED: "format-list-numbered",
  QUOTE: "format-quote",
  CODE: "code-tags",
  TABLE: "table",
  CHART: "chart-bar",
  CALCULATOR: "calculator",
  CALENDAR_CHECK: "calendar-check",
  CALENDAR_REMOVE: "calendar-remove",
  CALENDAR_PLUS: "calendar-plus",
  CALENDAR_MINUS: "calendar-minus",
  CALENDAR_TEXT: "calendar-text",
  CALENDAR_CLOCK: "calendar-clock",
  CALENDAR_ALERT: "calendar-alert",
  CALENDAR_STAR: "calendar-star",
  CALENDAR_HEART: "calendar-heart",
  CALENDAR_USER: "calendar-account",
  CALENDAR_EDIT: "calendar-edit",
  CALENDAR_DELETE: "calendar-remove",
  CALENDAR_EXPORT: "calendar-export",
  CALENDAR_IMPORT: "calendar-import",
  CALENDAR_SYNC: "calendar-sync",
  CALENDAR_REFRESH: "calendar-refresh",
  CALENDAR_SEARCH: "calendar-search",
  CALENDAR_FILTER: "calendar-filter",
  CALENDAR_SORT: "calendar-sort",
  CALENDAR_SETTINGS: "calendar-settings",
  CALENDAR_HELP: "calendar-help",
  CALENDAR_INFO: "calendar-information",
  CALENDAR_WARNING: "calendar-alert",
  CALENDAR_ERROR: "calendar-remove",
  CALENDAR_SUCCESS: "calendar-check",
  CALENDAR_QUESTION: "calendar-question",
  CALENDAR_COMMENT: "calendar-text",
  CALENDAR_LIKE: "calendar-heart",
  CALENDAR_DISLIKE: "calendar-remove",
  CALENDAR_SHARE: "calendar-share",
  CALENDAR_COPY: "calendar-content-copy",
  CALENDAR_CUT: "calendar-content-cut",
  CALENDAR_PASTE: "calendar-content-paste",
  CALENDAR_UNDO: "calendar-undo",
  CALENDAR_REDO: "calendar-redo",
  CALENDAR_BOLD: "calendar-format-bold",
  CALENDAR_ITALIC: "calendar-format-italic",
  CALENDAR_UNDERLINE: "calendar-format-underline",
  CALENDAR_STRIKETHROUGH: "calendar-format-strikethrough",
  CALENDAR_ALIGN_LEFT: "calendar-format-align-left",
  CALENDAR_ALIGN_CENTER: "calendar-format-align-center",
  CALENDAR_ALIGN_RIGHT: "calendar-format-align-right",
  CALENDAR_ALIGN_JUSTIFY: "calendar-format-align-justify",
  CALENDAR_LIST_BULLETED: "calendar-format-list-bulleted",
  CALENDAR_LIST_NUMBERED: "calendar-format-list-numbered",
  CALENDAR_QUOTE: "calendar-format-quote",
  CALENDAR_CODE: "calendar-code-tags",
  CALENDAR_TABLE: "calendar-table",
  CALENDAR_CHART: "calendar-chart-bar",
  CALENDAR_CALCULATOR: "calendar-calculator",
  CHEVRON_LEFT: "chevron-left",
  CHEVRON_RIGHT: "chevron-right",
  CHEVRON_DOUBLE_LEFT: "chevron-double-left",
  CHEVRON_DOUBLE_RIGHT: "chevron-double-right",
} as const;

export type AppIconsType = typeof AppIcons;
